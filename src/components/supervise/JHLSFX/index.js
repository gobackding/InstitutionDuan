...
class JHLSFX extends React.Component {
                    ...
                    this.state.pageBool ? <div style={{ padding: '10px' }} className="supervisorTableFY">
                        <div style={{ display: 'flex', justifyContent: "space-between" }}>
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                ...
                                <Button onClick={this.jiami.bind(this)}>加密</Button>
                            </div>
                            <div>
    ...
    jiami(){
        ...
        let name = this.$encryptionData('ni')
        console.log(JSON.parse(this.$decryptData(sessionStorage.getItem(name).split('"')[1])))
        // console.log(JSON.parse(this.$decryptData(sessionStorage.getItem(name))))
    }
    PopupDataLength(e) {
        this.setState({
