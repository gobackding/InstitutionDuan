...
class Historical extends React.Component {
    ...
    async componentDidMount() {
        ...
        let data = JSON.parse(this.$decryptData(localStorage.getItem(name).split('"')[1]))
        if (data.data) {
            ...
            if (!data.data) {
                ...
                let totleRuleCount = _this.formatNum(data.data.jhjglsList[0].TotleCount)
                let JCSJZLSum = _this.formatNum(data.data.jhjglsList[0].JCSJZLSum)
                let SFSJZLSum = _this.formatNum(data.data.jhjglsList[0].SFSJZLSum)
